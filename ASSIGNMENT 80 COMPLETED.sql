...
FROM EMPLOYEE_1000
JOIN DEPARTMENT  ON DEPARTMENT_ID = DEPARTMENT_IDS
WHERE DEPARTMENT_NAME = 'OPERATIONS';

/* 64. Modify the above query to display the employee numbers, last names, and salaries of all
...
/* 65.Display the names of all employees whose job title is the same as anyone in the sales
dept ? */
...
/* 66. Write a query to display the top three earners in the EMPLOYEES table. Display their last
names and salaries ? */
SELECT LAST_NAME, SALARY
FROM EMPLOYEE_1000
ORDER BY SALARY DESC
...
/* 67.  Display the names of all employees with their salary and commission earned. Employees
with a null commission should have O in the commission column ? */
SELECT FIRST_NAME || ' ' || LAST_NAME AS EMPLOYEE_NAME, SALARY,
COALESCE(COMMISSION_PCT, 0) AS COMMISIION_EARNED
FROM EMPLOYEE_1000 ;

/* 68. Display the Managers (name) with top three salaries along with their salaries and
...
/* 69. Write a compound query to produce a list of employees showing raise percentages,
...
/* 70. Display the structure of employee? */
DESCRIBE EMPLOYEE_1000 ;

/* 71. Display Firstname as ename in employee? */
SELECT FIRST_NAME AS ENAME FROM EMPLOYEE_1000;

/* 72 . Display the employee name in ascending order ? */
SELECT * FROM EMPLOYEE_1000 ORDER BY FIRST_NAME ASC ;


/*73.  DISPLAY the last name letter u ? */
SELECT * FROM EMPLOYEE_1000 WHERE LAST_NAME LIKE "U%";


/* 74. Find the date difference between the hire date and resignation_date for all the
...
/* 76.  Calcuate experience of the employee till date in Years and months(example 1 year and 3
months) ? */
...
SELECT
 ...
 /* 80. Find out the date that corresponds to the last Saturday of January, 2015 using with
clause? */
